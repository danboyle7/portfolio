...
function renderAbout(): CommandResult {
  ...
  lines.push("");
  lines.push(
    '<span class="term-yellow">TIP:</span> Run `profile` for a prettier overview!',
  );
  lines.push("");
...
function renderExperience(): CommandResult {
  ...
  for (const job of experience) {
    lines.push(
      `<span class="term-green font-bold">${job.role}</span> @ <span class="term-cyan">${job.company}</span>`,
    );
  ...
  lines.push(
    '<span class="term-dim">Run `experience` for the full interactive experience view</span>',
  );
  lines.push("");
...
function renderSkills(): CommandResult {
  ...
  lines.push(
    '<span class="term-dim">Run `skills` for the full interactive skills view</span>',
  );
  lines.push("");
...
function renderEducation(): CommandResult {
  ...
  for (const edu of education) {
    lines.push(
      `<span class="term-green font-bold">[*] ${edu.degree} in ${edu.field}</span>`,
    );
...
function renderBlogList(): CommandResult {
  ...
  lines.push(
    '<span class="term-dim">Run `blog` for the full blog listing</span>',
  );
  lines.push("");
...
function renderContact(): CommandResult {
  ...
  lines.push(
    `<span class="term-cyan">Location:</span> ${contact.locations.join(" | ")}`,
  );
  lines.push(
    `<span class="term-cyan">Status:</span>   <span class="term-green">${contact.availability}</span>`,
  );
  lines.push("");
...
function renderHobbies(): CommandResult {
  ...
  lines.push(
    '<span class="term-dim">Life isn\'t all about code... but mostly it is!</span>',
  );
  lines.push("");
...
function renderProjects(): CommandResult {
  ...
  for (const project of projects) {
    ...
    lines.push(
      `    <span class="term-magenta">Stack: ${project.technologies.join(" | ")}</span>`,
    );
    if (project.github) {
  ...
  lines.push(
    '<span class="term-dim">Run `projects` for the full interactive projects view</span>',
  );
  lines.push("");
