...
export const blogCommand: Command = {
  ...
  execute: (args): CommandResult => {
    ...
    for (let i = 0; i < posts.length; i++) {
      const post = posts[i]!;
      lines.push(
        `  <span class="term-green font-bold">[${i + 1}] ${post.title}</span>`,
      );
      lines.push(
        `      <span class="term-dim">${post.date} | ${post.readTime}</span>`,
      );
      lines.push(`      ${post.excerpt}`);
      lines.push(
        `      <span class="term-cyan">${post.tags.map((t) => `#${t}`).join(" ")}</span>`,
      );
...
function renderPost(post: BlogPost): CommandResult {
  ...
  if (post.content && post.content !== "Full article content...") {
    ...
    for (const line of contentLines) {
      ...
      if (line.startsWith("# ")) {
        lines.push(
          `<span class="term-green font-bold">${line.slice(2)}</span>`,
        );
        ...
        lines.push(
          `<span class="term-white font-bold">${line.slice(2, -2)}</span>`,
        );
      } else {
        lines.push(line);
      }
    ...
    lines.push("");
    lines.push(
      '<span class="term-dim">[Full article content would be rendered here]</span>',
    );
  }
