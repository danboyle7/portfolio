...
export const messageCommand: Command = {
  ...
  execute: (args): CommandResult => {
    ...
    if (!messageBody) {
      ...
      lines.push(
        '    message -t "Job Opportunity" "Would love to discuss a role..."',
      );
      lines.push("");
      lines.push(
        "  -------------------------------------------------------------------",
      ...
      lines.push("");
      lines.push(
        '  <span class="term-yellow">Alternative contact methods:</span>',
      );
      lines.push("");
      lines.push(
        `  * <span class="term-white">Email:</span>    <span class="term-green">${email}</span>`,
      );
      lines.push(
        '  * <span class="term-white">LinkedIn:</span> Great for professional connections',
      );
      lines.push(
        '  * <span class="term-white">GitHub:</span>   Check out my open source work',
      );
      lines.push("");
      lines.push(
        '  <span class="term-dim">Run `contact` to see all my social links!</span>',
      );
      lines.push("");
    ...
    addLine("", 0);
    addLine(
      '<span class="term-cyan font-bold">Preparing message...</span>',
      300,
    );
    addLine("", 200);
    ...
    addLine(
      `<span class="term-dim">Message:</span>  ${messageBody.length > 50 ? messageBody.slice(0, 47) + "..." : messageBody}`,
      200,
    );
    ...
    addLine(
      '<span class="term-yellow">[~] Connecting to mail server...</span>',
      400,
    );
    ...
    addLine("", 300);
    addLine(
      '<span class="term-green font-bold">[+] Message sent successfully!</span>',
      500,
    );
    addLine("", 300);
    addLine(
    ...
    addLine("", 200);
    addLine(
      '<span class="term-white">Thanks for reaching out! I\'ll get back to you soon.</span>',
      300,
    );
    addLine("", 200);
    addLine(
      '<span class="term-dim">Note: This is a simulated send. For a real response,</span>',
      100,
    );
    addLine(
      `<span class="term-dim">please email me directly at ${email}</span>`,
      100,
    );
    addLine("", 100);
